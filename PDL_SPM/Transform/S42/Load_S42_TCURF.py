...
schema = StructType([StructField('MANDT',IntegerType(),True),\
...
StructField('ABWCT',StringType(),True),\
StructField('ABWGA',StringType(),True),\
...
# MAGIC %sql
# MAGIC MERGE INTO S42.TCURF as T
# MAGIC USING (select * from (select RANK() OVER (PARTITION BY KURST,FCURR,TCURR,GDATU ORDER BY LandingFileTimeStamp DESC) as rn,* from stg_TCURF where MANDT = '100')A where A.rn = 1 ) as S 
...
# MAGIC T.ABWCT =  S.ABWCT,
# MAGIC T.ABWGA =  S.ABWGA,
