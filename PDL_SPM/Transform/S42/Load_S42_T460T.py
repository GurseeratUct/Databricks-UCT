...
df_T460T = spark.read.format(read_format) \
          .option("header", True) \
          .option("delimiter","^") \
          .schema(schema)  \
          .load(read_path)
...
df_add_column = df_T460T.withColumn('UpdatedOn',lit(current_timestamp())).withColumn('DataSource',lit('SAP')) #updatedOn
...
# MAGIC %sql
# MAGIC MERGE INTO S42.T460T  S
# MAGIC USING (select * from (select row_number() OVER (PARTITION BY MANDT,WERKS,SPRAS,SOBSL ORDER BY LandingFileTimeStamp DESC) as rn,* from stg_T460T)A where A.rn = 1 ) as T 
# MAGIC ON 
