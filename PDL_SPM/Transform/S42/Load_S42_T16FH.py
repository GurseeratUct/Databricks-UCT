...
StructField('FRGGT',StringType(),True),\
...
# MAGIC %sql
# MAGIC MERGE INTO S42.T16FH as T
# MAGIC USING (select * from (select ROW_NUMBER() OVER (PARTITION BY MANDT,SPRAS,FRGGR ORDER BY LandingFileTimeStamp DESC) as rn,* from stg_T16FH)A where A.rn = 1 ) as S 
# MAGIC ON T.`MANDT`	 = S.`MANDT`
# MAGIC and T.`SPRAS`	 = S.`SPRAS`
# MAGIC and T.`FRGGR`	 = S.`FRGGR`
# MAGIC WHEN MATCHED THEN
# MAGIC UPDATE SET
# MAGIC T.`MANDT`	 = S.`MANDT`,
# MAGIC T.`SPRAS`	 = S.`SPRAS`,
# MAGIC T.`FRGGR`	 = S.`FRGGR`,
# MAGIC T.`FRGGT`	 = S.`FRGGT`,
