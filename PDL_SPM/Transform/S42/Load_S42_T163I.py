...
StructField('KNTTX',StringType(),True),\
...
# MAGIC %sql
# MAGIC MERGE INTO S42.T163I as T
# MAGIC USING (select * from (select ROW_NUMBER() OVER (PARTITION BY MANDT,SPRAS,KNTTP ORDER BY LandingFileTimeStamp DESC) as rn,* from stg_T163I)A where A.rn = 1 ) as S 
# MAGIC ON T.`MANDT`	 = S.`MANDT`
# MAGIC and T.`SPRAS`	 = S.`SPRAS`
# MAGIC and T.`KNTTP`	 = S.`KNTTP`
# MAGIC WHEN MATCHED THEN
# MAGIC UPDATE SET
# MAGIC T.`MANDT`	 = S.`MANDT`,
# MAGIC T.`SPRAS`	 = S.`SPRAS`,
# MAGIC T.`KNTTP`	 = S.`KNTTP`,
# MAGIC T.`KNTTX`	 = S.`KNTTX`,
