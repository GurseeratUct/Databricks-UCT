...
# MAGIC %sql
# MAGIC MERGE INTO VE72.VENDOR as T
# MAGIC USING (select * from (select RANK() OVER (PARTITION BY ID ORDER BY LandingFileTimeStamp DESC) as rn,* from stg_VENDOR)A where A.rn = 1-- and A.LandingFileTimestamp IS NOT NULL 
# MAGIC ) as S 
# MAGIC ON T.ID = S.ID 
